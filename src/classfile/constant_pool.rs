...
pub enum CpEntry {
    ...
    MethodHandle { reference_kind: u8, reference: Box<CpEntry> },
    MethodType(String),                         // descriptor
    Dynamic       { bootstrap_attr_index: u16, name: String, descriptor: String },
    InvokeDynamic { bootstrap_attr_index: u16, name: String, descriptor: String },
    Module(String),
    Package(String),
}
...
impl ConstantPool {
    ...
    pub fn parse(cur: &mut Cursor) -> Result<Self> {
        ...
        while i < count {
            ...
            let entry = match tag {
                ...
                TAG_CLASS   => RawCpEntry::Class   { name_index:   cur.read_u16()? },
                TAG_STRING  => RawCpEntry::String  { string_index: cur.read_u16()? },
                TAG_FIELDREF           => RawCpEntry::Fieldref           { class_index: cur.read_u16()?, name_and_type_index: cur.read_u16()? },
                TAG_METHODREF          => RawCpEntry::Methodref          { class_index: cur.read_u16()?, name_and_type_index: cur.read_u16()? },
                TAG_INTERFACE_METHODREF=> RawCpEntry::InterfaceMethodref { class_index: cur.read_u16()?, name_and_type_index: cur.read_u16()? },
                TAG_NAME_AND_TYPE      => RawCpEntry::NameAndType        { name_index: cur.read_u16()?, descriptor_index: cur.read_u16()? },
                TAG_METHOD_HANDLE      => RawCpEntry::MethodHandle       { reference_kind: cur.read_u8()?, reference_index: cur.read_u16()? },
                TAG_METHOD_TYPE        => RawCpEntry::MethodType         { descriptor_index: cur.read_u16()? },
                TAG_DYNAMIC            => RawCpEntry::Dynamic            { bootstrap_method_attr_index: cur.read_u16()?, name_and_type_index: cur.read_u16()? },
                TAG_INVOKE_DYNAMIC     => RawCpEntry::InvokeDynamic      { bootstrap_method_attr_index: cur.read_u16()?, name_and_type_index: cur.read_u16()? },
                TAG_MODULE             => RawCpEntry::Module             { name_index: cur.read_u16()? },
                TAG_PACKAGE            => RawCpEntry::Package            { name_index: cur.read_u16()? },
                _ => return Err(DecompileError::InvalidCpTag { tag, index: i as u16 }),
            };
            raw.push(Some(entry));
            i += 1;
        }
        ...
        let get_utf8 = |raw: &Vec<Option<RawCpEntry>>, idx: u16| -> Result<String> {
            match raw.get(idx as usize).and_then(|e| e.as_ref()) {
                ...
                Some(_) => Err(DecompileError::CpTypeMismatch {
                    index: idx, expected: "Utf8", found: "other",
                }),
        ...
        for (idx, raw_opt) in raw.iter().enumerate() {
            let resolved = match raw_opt {
                ...
                Some(raw_entry) => Some(match raw_entry {
                    ...
                    RawCpEntry::Class   { name_index }   => CpEntry::Class(get_utf8(&raw, *name_index)?),
                    RawCpEntry::String  { string_index } => CpEntry::String(get_utf8(&raw, *string_index)?),
                    RawCpEntry::Module  { name_index }   => CpEntry::Module(get_utf8(&raw, *name_index)?),
                    RawCpEntry::Package { name_index }   => CpEntry::Package(get_utf8(&raw, *name_index)?),
                    RawCpEntry::MethodType { descriptor_index } =>
                        CpEntry::MethodType(get_utf8(&raw, *descriptor_index)?),
                    RawCpEntry::NameAndType { name_index, descriptor_index } =>
                        CpEntry::NameAndType {
                            name:       get_utf8(&raw, *name_index)?,
                            descriptor: get_utf8(&raw, *descriptor_index)?,
                        },
                    RawCpEntry::Fieldref { class_index, name_and_type_index }
                    | RawCpEntry::Methodref { class_index, name_and_type_index }
                    | RawCpEntry::InterfaceMethodref { class_index, name_and_type_index } => {
                        let class_name = match raw.get(*class_index as usize).and_then(|e| e.as_ref()) {
                            Some(RawCpEntry::Class { name_index }) => get_utf8(&raw, *name_index)?,
                            _ => return Err(DecompileError::CpIndexOutOfRange(*class_index)),
                        };
                        let (name, descriptor) = match raw.get(*name_and_type_index as usize).and_then(|e| e.as_ref()) {
                            Some(RawCpEntry::NameAndType { name_index, descriptor_index }) =>
                                (get_utf8(&raw, *name_index)?, get_utf8(&raw, *descriptor_index)?),
                            _ => return Err(DecompileError::CpIndexOutOfRange(*name_and_type_index)),
                        };
                        let mr = MemberRef { class_name, name, descriptor };
                        match raw_entry {
                            RawCpEntry::Fieldref { .. }           => CpEntry::Fieldref(mr),
                            RawCpEntry::Methodref { .. }          => CpEntry::Methodref(mr),
                            RawCpEntry::InterfaceMethodref { .. } => CpEntry::InterfaceMethodref(mr),
                            _ => unreachable!(),
                        }
                    }
                    RawCpEntry::Dynamic { bootstrap_method_attr_index, name_and_type_index }
                    | RawCpEntry::InvokeDynamic { bootstrap_method_attr_index, name_and_type_index } => {
                        let (name, descriptor) = match raw.get(*name_and_type_index as usize).and_then(|e| e.as_ref()) {
                            Some(RawCpEntry::NameAndType { name_index, descriptor_index }) =>
                                (get_utf8(&raw, *name_index)?, get_utf8(&raw, *descriptor_index)?),
                            _ => return Err(DecompileError::CpIndexOutOfRange(*name_and_type_index)),
                        ...
                        match raw_entry {
                            RawCpEntry::Dynamic { .. } =>
                                CpEntry::Dynamic { bootstrap_attr_index: *bootstrap_method_attr_index, name, descriptor },
                            _ =>
                                CpEntry::InvokeDynamic { bootstrap_attr_index: *bootstrap_method_attr_index, name, descriptor },
                        }
                    }
                    RawCpEntry::MethodHandle { reference_kind, reference_index } => {
                        // Resolve the referenced Methodref/Fieldref/InterfaceMethodref entry.
                        let referenced = raw.get(*reference_index as usize)
                            .and_then(|e| e.as_ref());
                        let resolved_ref = match referenced {
                            Some(RawCpEntry::Fieldref { class_index, name_and_type_index })
                            | Some(RawCpEntry::Methodref { class_index, name_and_type_index })
                            | Some(RawCpEntry::InterfaceMethodref { class_index, name_and_type_index }) => {
                                let class_name = match raw.get(*class_index as usize).and_then(|e| e.as_ref()) {
                                    Some(RawCpEntry::Class { name_index }) => get_utf8(&raw, *name_index)?,
                                    _ => return Err(DecompileError::CpIndexOutOfRange(*class_index)),
                                };
                                ...
                                match referenced {
                                    ...
                                    Some(RawCpEntry::InterfaceMethodref { .. }) => CpEntry::InterfaceMethodref(mr),
                                    _ => unreachable!(),
                                }
                            }
            ...
            if idx == 0 { continue; } // skip 0 → already pushed once
        }
    ...
    pub fn utf8(&self, index: u16) -> Result<&str> {
        match self.get(index)? {
            ...
            _ => Err(DecompileError::CpTypeMismatch { index, expected: "Utf8", found: "other" }),
        }
    }
    ...
    pub fn class_name(&self, index: u16) -> Result<&str> {
        match self.get(index)? {
            ...
            _ => Err(DecompileError::CpTypeMismatch { index, expected: "Class", found: "other" }),
        }
    }
