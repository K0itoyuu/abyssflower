...
pub enum Attribute {
    ...
    EnclosingMethod { class_name: String, method_name: Option<String>, method_descriptor: Option<String> },
    Synthetic,
    Signature(String),
    SourceFile(String),
...
pub fn parse_attributes(cur: &mut Cursor, pool: &ConstantPool, count: u16, version: (u16, u16)) -> Result<Vec<Attribute>> {
...
fn parse_one(cur: &mut Cursor, pool: &ConstantPool, version: (u16, u16)) -> Result<Attribute> {
    ...
    let attr = match name.as_str() {
        ...
        "RuntimeVisibleAnnotations"           => Attribute::RuntimeVisibleAnnotations(parse_annotations(&mut attr_cur, pool)?),
        "RuntimeInvisibleAnnotations"         => Attribute::RuntimeInvisibleAnnotations(parse_annotations(&mut attr_cur, pool)?),
        "RuntimeVisibleParameterAnnotations"  => Attribute::RuntimeVisibleParameterAnnotations(parse_parameter_annotations(&mut attr_cur, pool)?),
        "RuntimeInvisibleParameterAnnotations"=> Attribute::RuntimeInvisibleParameterAnnotations(parse_parameter_annotations(&mut attr_cur, pool)?),
        "AnnotationDefault"                   => Attribute::AnnotationDefault(parse_element_value(&mut attr_cur, pool)?),
        ...
        "NestHost"                            => { let idx = attr_cur.read_u16()?; Attribute::NestHost(pool.class_name(idx)?.to_string()) },
        ...
        "StackMapTable"                       => Attribute::StackMapTable(StackMapTable { raw_bytes: attr_cur.remaining_slice().to_vec() }),
        _ => Attribute::Unknown { name, bytes: attr_cur.remaining_slice().to_vec() },
    };
...
fn parse_code(cur: &mut Cursor, pool: &ConstantPool, version: (u16, u16)) -> Result<Attribute> {
    ...
    for _ in 0..ex_count {
        ...
        exception_table.push(ExceptionHandler { start_pc, end_pc, handler_pc, catch_type });
    }

    let attr_count = cur.read_u16()?;
    let attributes = parse_attributes(cur, pool, attr_count, version)?;

    Ok(Attribute::Code(CodeAttribute { max_stack, max_locals, instructions, exception_table, attributes }))
}

fn parse_constant_value(cur: &mut Cursor) -> Result<Attribute> {
    let index = cur.read_u16()?;
    Ok(Attribute::ConstantValue(ConstantValueAttribute { constant_value_index: index }))
}
...
fn parse_exceptions(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    Ok(Attribute::Exceptions(ExceptionsAttribute { exception_names: names }))
}
...
fn parse_inner_classes(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    for _ in 0..count {
        ...
        classes.push(InnerClassInfo {
            inner_class_name:   if inner_idx == 0 { None } else { Some(pool.class_name(inner_idx)?.to_string()) },
            outer_class_name:   if outer_idx == 0 { None } else { Some(pool.class_name(outer_idx)?.to_string()) },
            simple_name:        if name_idx  == 0 { None } else { Some(pool.utf8(name_idx)?.to_string()) },
...
fn parse_enclosing_method(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    let (method_name, method_descriptor) = if method_idx == 0 {
        ...
        match pool.get(method_idx)? {
            ...
            _ => return Err(DecompileError::CpTypeMismatch {
                index: method_idx, expected: "NameAndType", found: "other",
            }),
        }
    };
    Ok(Attribute::EnclosingMethod { class_name, method_name, method_descriptor })
}
...
fn parse_line_number_table(cur: &mut Cursor) -> Result<Attribute> {
    ...
    for _ in 0..count {
        table.push(LineNumber { start_pc: cur.read_u16()?, line_number: cur.read_u16()? });
    }
...
fn parse_bootstrap_methods(cur: &mut Cursor) -> Result<Attribute> {
    ...
    for _ in 0..count {
        ...
        for _ in 0..arg_count { args.push(cur.read_u16()?); }
        methods.push(BootstrapMethod { bootstrap_method_ref: ref_idx, arguments: args });
    }
...
fn parse_method_parameters(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    for _ in 0..count {
        let name_idx = cur.read_u16()?;
        let flags    = cur.read_u16()?;
        let name = if name_idx == 0 { None } else { Some(pool.utf8(name_idx)?.to_string()) };
        params.push(MethodParameter { name, access_flags: flags });
    }
...
fn parse_record(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    for _ in 0..count {
        ...
        components.push(RecordComponent { name, descriptor, attributes });
    }
...
fn parse_module(cur: &mut Cursor, pool: &ConstantPool) -> Result<Attribute> {
    ...
    let module_name = match pool.get(mod_idx)? {
        ...
        _ => return Err(DecompileError::CpTypeMismatch { index: mod_idx, expected: "Module", found: "other" }),
    };
    let module_flags = cur.read_u16()?;
    let ver_idx  = cur.read_u16()?;
    let module_version = if ver_idx == 0 { None } else { Some(pool.utf8(ver_idx)?.to_string()) };

    let req_count = cur.read_u16()? as usize;
    let mut requires = Vec::with_capacity(req_count);
    for _ in 0..req_count {
        let ridx  = cur.read_u16()?;
        let rname = match pool.get(ridx)? { CpEntry::Module(s) => s.clone(), _ => String::new() };
        let flags = cur.read_u16()?;
        let vidx  = cur.read_u16()?;
        let ver   = if vidx == 0 { None } else { Some(pool.utf8(vidx)?.to_string()) };
        requires.push(ModuleRequires { module_name: rname, flags, version: ver });
    }
    ...
    for _ in 0..exp_count {
        ...
        let pname = match pool.get(pidx)? { CpEntry::Package(s) => s.clone(), _ => String::new() };
        let flags = cur.read_u16()?;
        let to_count = cur.read_u16()? as usize;
        let mut to = Vec::with_capacity(to_count);
        for _ in 0..to_count {
            let tidx = cur.read_u16()?;
            to.push(match pool.get(tidx)? { CpEntry::Module(s) => s.clone(), _ => String::new() });
        }
        exports.push(ModuleExports { package_name: pname, flags, to_modules: to });
    }
    ...
    for _ in 0..open_count {
        ...
        opens.push(ModuleOpens { package_name: pname, flags, to_modules: to });
    }

    let use_count = cur.read_u16()? as usize;
    let mut uses = Vec::with_capacity(use_count);
    for _ in 0..use_count { uses.push(pool.class_name(cur.read_u16()?)?.to_string()); }
    ...
    for _ in 0..prov_count {
        ...
        for _ in 0..wc { with.push(pool.class_name(cur.read_u16()?)?.to_string()); }
        provides.push(ModuleProvides { service_name: svc, with });
    }

    Ok(Attribute::Module(ModuleAttribute { module_name, module_flags, module_version, requires, exports, opens, uses, provides }))
}
...
fn parse_annotations(cur: &mut Cursor, pool: &ConstantPool) -> Result<Vec<Annotation>> {
    ...
    for _ in 0..count { anns.push(parse_annotation(cur, pool)?); }
    Ok(anns)
}
...
fn parse_element_value(cur: &mut Cursor, pool: &ConstantPool) -> Result<ElementValue> {
    ...
    use crate::classfile::constant_pool::CpEntry;
    let val = match tag {
        'B' => { let idx = cur.read_u16()?; ElementValue::Byte(match pool.get(idx)? { CpEntry::Integer(v) => *v, _ => 0 }) }
        'C' => { let idx = cur.read_u16()?; ElementValue::Char(match pool.get(idx)? { CpEntry::Integer(v) => *v, _ => 0 }) }
        'D' => { let idx = cur.read_u16()?; ElementValue::Double(match pool.get(idx)? { CpEntry::Double(v) => *v, _ => 0.0 }) }
        'F' => { let idx = cur.read_u16()?; ElementValue::Float(match pool.get(idx)? { CpEntry::Float(v) => *v, _ => 0.0 }) }
        'I' => { let idx = cur.read_u16()?; ElementValue::Int(match pool.get(idx)? { CpEntry::Integer(v) => *v, _ => 0 }) }
        'J' => { let idx = cur.read_u16()?; ElementValue::Long(match pool.get(idx)? { CpEntry::Long(v) => *v, _ => 0 }) }
        'S' => { let idx = cur.read_u16()?; ElementValue::Short(match pool.get(idx)? { CpEntry::Integer(v) => *v, _ => 0 }) }
        'Z' => { let idx = cur.read_u16()?; ElementValue::Boolean(match pool.get(idx)? { CpEntry::Integer(v) => *v != 0, _ => false }) }
        's' => { let idx = cur.read_u16()?; ElementValue::String(pool.utf8(idx)?.to_string()) }
        'e' => {
            let type_idx  = cur.read_u16()?;
        ...
        'c' => { let idx = cur.read_u16()?; ElementValue::ClassInfo(pool.utf8(idx)?.to_string()) }
        '@' => ElementValue::Annotation(parse_annotation(cur, pool)?),
        '[' => {
            let count = cur.read_u16()? as usize;
            let mut arr = Vec::with_capacity(count);
            for _ in 0..count { arr.push(parse_element_value(cur, pool)?); }
            ElementValue::Array(arr)
        }
        _ => return Err(DecompileError::MalformedAttribute("Annotation", format!("unknown element_value tag '{}'", tag))),
    };
    Ok(val)
}

fn parse_parameter_annotations(cur: &mut Cursor, pool: &ConstantPool) -> Result<Vec<Vec<Annotation>>> {
    let param_count = cur.read_u8()? as usize;
    let mut result = Vec::with_capacity(param_count);
    for _ in 0..param_count { result.push(parse_annotations(cur, pool)?); }
    Ok(result)
}
