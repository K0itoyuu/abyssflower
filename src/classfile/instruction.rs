        ...
        match self {
            ...
            InsnKind::TableSwitch {
                ...
                let mut targets: Vec<u32> =
                    offsets.iter().map(|o| (base + *o as i64) as u32).collect();
...
fn decode_operands(
    ...
    let kind = match opcode {
        ...
        | if_icmpgt | if_icmple | if_acmpeq | if_acmpne | ifnull | ifnonnull | jsr => Branch {
            offset: cur.read_i16()? as i32,
        },
        goto => Branch {
            offset: cur.read_i16()? as i32,
        },
        ...
        getstatic | putstatic | getfield | putfield | new | anewarray | checkcast | instanceof => {
            Cp {
                index: cur.read_u16()?,
            }
        }
        ...
        // ── invocations ───────────────────────────────────────────────
        invokevirtual | invokespecial | invokestatic => Invoke {
            index: cur.read_u16()?,
            count: 0,
        },
        invokeinterface => {
        ...
        tableswitch => {
            ...
            if count <= 0 || count as u64 > (cur.remaining() / 4) as u64 {
                return Err(DecompileError::MalformedAttribute(
                    "Code",
                    format!("invalid tableswitch bounds {low}..={high} at offset {insn_offset}"),
                ));
            }
        ...
        lookupswitch => {
            ...
            cur.skip(align as usize)?;
            let default_offset = cur.read_i32()?;
            let npairs = cur.read_i32()?;
            if npairs < 0 || npairs as u64 > (cur.remaining() / 8) as u64 {
                return Err(DecompileError::MalformedAttribute(
                    "Code",
                    format!("invalid lookupswitch pair count {npairs} at offset {insn_offset}"),
                ));
            }
...
mod tests {
    ...
    #[test]
    fn rejects_negative_lookup_switch_count() {
        let mut code = vec![opc::lookupswitch, 0, 0, 0];
    ...
    #[test]
    fn rejects_table_switch_count_larger_than_payload() {
        let mut code = vec![opc::tableswitch, 0, 0, 0];
        code.extend_from_slice(&0_i32.to_be_bytes());
        code.extend_from_slice(&0_i32.to_be_bytes());
    ...
    #[test]
    fn rejects_empty_table_switch_range() {
        let mut code = vec![opc::tableswitch, 0, 0, 0];
        code.extend_from_slice(&0_i32.to_be_bytes());
        code.extend_from_slice(&1_i32.to_be_bytes());
        code.extend_from_slice(&0_i32.to_be_bytes());
    ...
    #[test]
    fn rejects_reversed_table_switch_bounds() {
        let mut code = vec![opc::tableswitch, 0, 0, 0];
        code.extend_from_slice(&0_i32.to_be_bytes());
        code.extend_from_slice(&1_i32.to_be_bytes());
        code.extend_from_slice(&0_i32.to_be_bytes());
    ...
    #[test]
    fn rejects_lookup_switch_count_larger_than_payload() {
        let mut code = vec![opc::lookupswitch, 0, 0, 0];
        code.extend_from_slice(&0_i32.to_be_bytes());
        code.extend_from_slice(&2_i32.to_be_bytes());
        code.extend_from_slice(&1_i32.to_be_bytes());
        code.extend_from_slice(&4_i32.to_be_bytes());
        assert!(decode(&code).is_err());
    }
}
